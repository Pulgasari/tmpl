...
dom.attrList = (spec)          => Array.from(spec.attributes),
...
function expandToDiv (el, tag) {
  ...
  for (const attr of Array.from(el.attributes)) {
    ...
    if (attr.name === '$attr') {
      div.appendChild(don.text(attr.value));
    } else {
      div.setAttribute(attr.name, attr.value);
    }
  }

  for (const { name, value } of dom.attrList(el)) switch (name) {
    case 'class' : continue;
    case '$attr' : div.appendChild(don.text(value));
