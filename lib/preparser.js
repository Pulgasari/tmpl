...
const REGEXP_EVENT = /\bon:([a-zA-Z]+)\s*=\s*\{([^}]*)\}/g;
function processEventBindings (code) {
  return code.replace(REGEXP_EVENT, (_, ev, expr) => {
...
function processPositionalArgs (code) {
  return code.replace(REGEXP_POS_ARGS, (_, tagStart, _quote, val) => {
...
function processMultiAttr (code) {
  return code.replace(REGEXP_MULTI_ATTR, (_, attrs, quote, val) => {
    return attrs
      .split(',')
      .map((a) => a.trim())
      .filter(Boolean)
...
const REGEXP_SELF_CLOSING = /<([a-zA-Z0-9-]+)([^>]*?)\/>/g;
function processSelfClosingTags (code) {
  return code.replace(REGEXP_SELF_CLOSING, (match, tag, attrs) => {
