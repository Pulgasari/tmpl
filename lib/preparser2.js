...
function parseTag(code, start) {
  ...
  // Closing tags are intentionally left untouched.
  if (code.charCodeAt(pos) === CHAR_SLASH) {
    return null;
  }

  // Only handle tag names matching the old regex:
  // [a-zA-Z0-9_-]+
  const tagStart = pos;
  ...
  // Optional shorthand selectors directly after the tag name:
  //
  // <tag.foo.bar#id
  //
  // The old implementation allowed any combination of .foo / #bar.
  let classes = '';
  let id = '';
  ...
  while (pos < length) {
    ...
    // Must have a selector name to be valid shorthand.
    if (pos === nameStart) {
      // Keep the shorthand parser from swallowing malformed input.
      pos--;
      break;
    }
    ...
    if (marker === CHAR_DOT) {
      ...
      // Preserve old semantics: the last #selector wins.
      id = name;
    }
  }

  // Find the end of the tag while respecting:
  // - quoted attribute values
  // - {...} event expressions
  //
  // This is intentionally a small scanner, not a full HTML parser.
  const attrStart = pos;
  ...
  // No closing ">" => not a complete tag; leave it untouched.
  if (pos >= length) {
    return null;
  }
  ...
  // Detect self-closing syntax.
  //
  // We intentionally inspect trailing whitespace + "/" rather than relying
  // on /...\/>/ so we do not accidentally look inside quoted values.
  ...
  // Rebuild the tag with all supported shorthand expanded.
